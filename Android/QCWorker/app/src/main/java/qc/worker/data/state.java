package qc.worker.data;

import android.content.Context;
import android.graphics.Bitmap;


public class state {
    static public String UUID="";
    static public String level="";
    static public String site="";
    static public Double latitude=0.0;
    static public Double longitude=0.0;
    static public String HostUrl="http://www.qualityconstruction.be/app/";
    static public Context context=null;
    static public String workerName="";
    static public String CompanyURL="http://www.qualityconstruction.be";
    static public String loadedfragment="";
}
