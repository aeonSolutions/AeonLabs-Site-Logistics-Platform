...
foreach( $exploded as $request):
	...
	if($request=='ausencias'):
		if(isset($data['startdate']) and isset($data['enddate'])):
			$str=" inicio>='".$data['startdate']."' and fim<='".$data['enddate']."'";
		else:
			$str="";
		endif;
		$worker = isset($data['cod']) ? "where cod_worker='".$data['cod']."'" : "";
		if($worker=="" and $str<>""):
			$str="where ".$str;
		elseif($worker<>"" and $str<>""):
			$str=" and ".$str;
		endif;

		$query=$db->getquery("select cod_ausencia, cod_worker, inicio, fim, tipo, viagem, motivo from worker_ausencia ".$worker.$str);
		$notfound=false;
	endif;
