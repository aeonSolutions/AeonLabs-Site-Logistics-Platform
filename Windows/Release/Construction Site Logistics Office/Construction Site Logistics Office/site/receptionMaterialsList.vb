...
Public Class receptionMaterials_frm
    ...
    Private Sub receptionMaterials_frm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        MainMdiForm.childForm = "receptionMaterialsList"
...
lastLine:
    ...
    Private Sub bwLoadAM_DoWork(sender As Object, e As DoWorkEventArgs) Handles bwload.DoWork
        ...
        If Not IsResponseOk(response) Then
            ...
            GoTo lastLine
        End If
        works_materials = request.ConvertDataToArray("sitematreception", state.querySiteMaterialsReceptionFields, response)
...
lastLine:
        ...
        ReDim AMTable(works_materials.Item("cod_materials_reception").Count - 1 + pos, 6)
        For i = 0 To works_materials.Item("cod_materials_reception").Count - 1 + pos
            For j = 0 To 6
                AMTable(i, j) = ""
            Next j
        Next i
        ...
        For i = 0 To works_materials.Item("cod_materials_reception").Count - 1
            AMTable(pos + i, 0) = works_materials.Item("material")(i)
            AMTable(pos + i, 1) = works_materials.Item("data")(i)
            AMTable(pos + i, 2) = works_materials.Item("start")(i)
            AMTable(pos + i, 3) = works_materials.Item("end")(i)
            AMTable(pos + i, 4) = works_materials.Item("qtd")(i)
            AMTable(pos + i, 5) = works_materials.Item("units")(i)
            AMTable(pos + i, 6) = works_materials.Item("notas")(i)
        Next i
        s(2) = works_materials.Item("cod_materials_reception").Count
        e.Result = s
    ...
    Private Sub bwLoadAM_RunWorkerCompleted(sender As Object, e As RunWorkerCompletedEventArgs) Handles bwload.RunWorkerCompleted
        ...
        With datatable
            ...
            .Columns(0).HeaderCell.Value = translations.getText("receptionMaterialsMaterial")
            .Columns(0).Width = 400
            ...
            .Columns(2).HeaderCell.Value = translations.getText("dateStartTitle")
            .Columns(2).Width = g.MeasureString("31-31-2031", fontToMeasure).Width

            .Columns(3).HeaderCell.Value = translations.getText("dateEndTitle")
            .Columns(3).Width = g.MeasureString("31-31-2031", fontToMeasure).Width

            .Columns(6).HeaderCell.Value = translations.getText("AnnotationTitle")
            .Columns(6).Width = 400
