...
Public Class workers_frm
    ...
    Private Sub drawControls()
        ...
        save_limosa.Font = New Font(state.fontText.Families(0), state.RegularTextFontSize, Drawing.FontStyle.Regular)
        del_limosa.Font = New Font(state.fontText.Families(0), state.RegularTextFontSize, Drawing.FontStyle.Regular)
        del_files.Font = New Font(state.fontText.Families(0), state.RegularTextFontSize, Drawing.FontStyle.Regular)
        addCalendarDateLink.Font = New Font(state.fontText.Families(0), state.RegularTextFontSize, Drawing.FontStyle.Regular)
        download.Font = New Font(state.fontText.Families(0), state.RegularTextFontSize, Drawing.FontStyle.Regular)
        ...
        save_limosa.Text = translations.getText("saveLink")
        del_limosa.Text = translations.getText("delLink")
        del_files.Text = translations.getText("delLink")
        addCalendarDateLink.Text = translations.getText("addCalendarDateLink")
        download.Text = translations.getText("downloadLink")
        ...
        naturalidade_lbl.Text = translations.getText("nationality") & "*"
        ...
        country_lbl.Text = translations.getText("country") & "*"
        ...
        save_limosa.Location = New Point(GroupBoxLimosas.Width - save_limosa.Width - 5, save_limosa.Location.Y)
        del_limosa.Location = New Point(save_limosa.Location.X - save_limosa.Width - 5, del_limosa.Location.Y)

        Me.ResumeLayout()
        Refresh()
    End Sub

    Private Sub workers_frm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        state = MainMdiForm.state
        translations = New languageTranslations(state)
        MainMdiForm.childForm = "workers"
        delWorkerCard.Enabled = False
        drawControls()
    End Sub

    Private Sub workers_frm_Shown(sender As Object, e As EventArgs) Handles MyBase.Shown
        load_list()
        If (Directory.Exists(MainMdiForm.state.basePath & "tmp")) Then
            For Each deleteFile In Directory.GetFiles(MainMdiForm.state.basePath & "tmp", "*.*", SearchOption.TopDirectoryOnly)
                File.Delete(deleteFile)
            Next
        Else
            Try
                Directory.CreateDirectory(MainMdiForm.state.basePath & "tmp")
            Catch ex As Exception
                msgbox = New message_box_frm("tem que criar manualmente a pasta: " & vbCrLf & MainMdiForm.state.basePath & "tmp", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Error)
                msgbox.ShowDialog()
            End Try
...
lastLine:
    ...
    Private Sub bwSites_RunWorkerCompleted(sender As Object, e As RunWorkerCompletedEventArgs) Handles bwSites.RunWorkerCompleted
        ...
        contratoImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileMissingSmall.fw.png"))
        destacamentoImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileMissingSmall.fw.png"))
        actImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileMissingSmall.fw.png"))
        a1Img.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileMissingSmall.fw.png"))
        mutuelleImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileMissingSmall.fw.png"))
        medicoImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileMissingSmall.fw.png"))
        gruistaImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileMissingSmall.fw.png"))
        removeMask()
        Me.ResumeLayout()
        Refresh()
        loaded = True
    ...
    Private Sub delWorkerCard_Click(sender As Object, e As EventArgs) Handles delWorkerCard.Click
        ...
        If listview_works.SelectedIndex < 1 Then
            ...
            translations.load("messagebox")
            msgbox = New message_box_frm(messageTxt, translations.getText("information"), MessageBoxButtons.OK, MessageBoxIcon.Information, Me.Location.X + Me.Width / 2, Me.Location.Y + Me.Height / 2)
            msgbox.ShowDialog()
            Exit Sub
        End If
        ...
        translations.load("messagebox")
        msgbox = New message_box_frm(messageTxt & ", " & works_worker.Item("name")(posToLoad) & "?", translations.getText("question"), MessageBoxButtons.OK, MessageBoxIcon.Question, Me.Location.X + Me.Width / 2, Me.Location.Y + Me.Height / 2)
        If (msgbox.ShowDialog = DialogResult.OK) Then
            ...
            End If
            translations.load("messagebox")
            msgbox = New message_box_frm(GetMessage(response), translations.getText("information"), MessageBoxButtons.OK, MessageBoxIcon.Information, Me.Location.X + Me.Width / 2, Me.Location.Y + Me.Height / 2)
            msgbox.ShowDialog()
            ClearForm()
            load_list()
        End If
    ...
    Private Sub saveWorkerCard_Click(sender As Object, e As EventArgs) Handles saveWorkerCard.Click
        ...
        If (ValidationRequired) Then
            ...
            translations.load("messagebox")
            msgbox = New message_box_frm(messageTxt & ", " & err & ".", translations.getText("exclamation"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation, Me.Location.X + Me.Width / 2, Me.Location.Y + Me.Height / 2)
            msgbox.ShowDialog()
        Else
            Dim aloja As String = ""
            Dim ativ As String = ""
            ...
            Else
                prevSelection = 0
                translations.load("messagebox")
                messageTxt = translations.getText("resultSuccessSaveRecord")
                msgbox = New message_box_frm(messageTxt, translations.getText("information"), MessageBoxButtons.OK, MessageBoxIcon.Information, Me.Location.X + Me.Width / 2, Me.Location.Y + Me.Height / 2)
                msgbox.ShowDialog()
            End If
            ...
            If (listview_works.SelectedItems.Count > 0) Then
                If listview_works.SelectedIndices(0) > 0 Then 'edit worker
                    If Not cod.Equals(works_worker.Item("cod_worker")(posToLoad)) Then
                        translations.load("errorMessages")
                        messageTxt = translations.getText("errorSavingData")
                        translations.load("messagebox")
                        msgbox = New message_box_frm(messageTxt, translations.getText("exclamation"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation, Me.Location.X + Me.Width / 2, Me.Location.Y + Me.Height / 2)
                        msgbox.ShowDialog()
                        Exit Sub
                    End If
                End If
            ...
            uploadWorkerFile(cod, contratofile.Text, "contrato")
            uploadWorkerFile(cod, destacamentofile.Text, "destacamento")
            uploadWorkerFile(cod, a1file.Text, "a1")
            uploadWorkerFile(cod, mutuellefile.Text, "mutuelle")
            uploadWorkerFile(cod, medicofile.Text, "medico")
            uploadWorkerFile(cod, gruistaFile.Text, "gruista")
            uploadWorkerFile(cod, ccfile.Text, "cc")
            uploadWorkerFile(cod, csaudefile.Text, "csaude")
    ...
    Private Function uploadWorkerFile(cod As String, filename As String, ParamRequest As String) As Boolean
        ...
        If (File.Exists(filename)) Then
            ...
            vars.Add("file", ParamRequest)
            vars.Add("user", cod)
            Dim request As New HttpData(state)
            response = request.SendHttpFile(vars, filename)
            If Not IsResponseOk(response) Then
                If Not IsNothing(MainMdiForm.busy) Then
                    If MainMdiForm.busy.isActive().Equals(True) Then
                        MainMdiForm.busy.Close(True)
    ...
    Private Sub ClearForm()
        ...
        medicoImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileMissingSmall.fw.png"))
        gruistaImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileMissingSmall.fw.png"))
        ccimg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileMissingSmall.fw.png"))
        csaudeimg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileMissingSmall.fw.png"))
        qrcode_img.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("searchqrcode.png"))
        ...
        If (listview_works.SelectedItems.Count > 0) Then
            ...
            If listview_works.SelectedIndex > 0 Then
                ...
                If (OpenFileDialog1.ShowDialog() = DialogResult.OK) Then
                    ...
                    translations.load("messagebox")
                    msgbox = New message_box_frm(translations.getText("questionAddPhoto"), translations.getText("question"), MessageBoxButtons.OK, MessageBoxIcon.Question, Me.Location.X + Me.Width / 2, Me.Location.Y + Me.Height / 2)
                    If (msgbox.ShowDialog = DialogResult.OK) Then
                        ...
                        Else
                            ...
                            Dim tImage As Bitmap = Bitmap.FromStream(New MemoryStream(tClient.DownloadData(state.ServerBaseAddr & "/photos/" & works_worker.Item("photo")(idx - 1))))
                            photobox.Image = tImage
                            photobox.SizeMode = PictureBoxSizeMode.StretchImage
                            listview_works.SelectedIndex = idx
                        End If
                        enableButtonsAndLinks(Me, False)
    ...
    Private Sub contratoImg_Click(sender As Object, e As EventArgs) Handles contratoImg.Click
        If (listview_works.SelectedItems.Count > 0) Then
            If (listview_works.SelectedIndex > 0) Then
                If (Not works_worker.Item("contrato_file")(listview_works.SelectedIndices(0)).Equals("")) Then
                    Dim vars As New Dictionary(Of String, String)
                    vars.Add("task", "26") 'getfile
        ...
        End If
    End Sub
    Private Sub Del_limosa_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles del_limosa.LinkClicked

        translations.load("messagebox")
        Dim messageTxt = translations.getText("questionRemoveLimosa")
        translations.load("messagebox")
        msgbox = New message_box_frm(messageTxt & ". ", translations.getText("question"), MessageBoxButtons.OK, MessageBoxIcon.Question, Me.Location.X + Me.Width / 2, Me.Location.Y + Me.Height / 2)
        Dim response As String
        If (msgbox.ShowDialog = DialogResult.OK) Then
            enableButtonsAndLinks(Me, False)
            ...
            Else
                ...
                qrcode_img.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("searchqrcode.png"))
                limosasImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileMissingSmall.fw.png"))
                If workersListSelection.SelectedIndex.Equals(0) Then
                    posToLoad = listview_works.SelectedIndices(0) - 1
                ElseIf workersListSelection.SelectedIndex.Equals(1) Then
                    posToLoad = listview_works.SelectedIndices(0) - 1 + inactiveWorkerPosZero
                ...
                If IsNothing(limosasDB) Then
                    translations.load("workers")
                    limosaList.Items.Clear()
                    limosaList.Items.Add(translations.getText("dropboxAddLimosa"))
                Else
    ...
    Private Sub save_limosa_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles save_limosa.LinkClicked
        Dim response As String
        ...
        Else
            ...
            Dim request As New HttpData(state)
            response = request.SendHttpFile(vars, limosafile.Text)
            If Not IsNothing(MainMdiForm.busy) Then
                If MainMdiForm.busy.isActive().Equals(True) Then
                    MainMdiForm.busy.Close(True)
                End If
            ...
            If Not IsResponseOk(response) Then
                translations.load("messagebox")
                msgbox = New message_box_frm(GetMessage(response), translations.getText("exclamantion"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation, Me.Location.X + Me.Width / 2, Me.Location.Y + Me.Height / 2)
                msgbox.ShowDialog()
            Else
                translations.load("messagebox")
                ...
                If (File.Exists(QrCode)) Then
                    ...
                    Dim request2 As New HttpData(state)
                    response = request2.SendHttpFile(vars2, QrCode)
    ...
    Private Sub del_files_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles del_files.LinkClicked
        ...
        If Not enableDel Then
            ...
            translations.load("messagebox")
            msgbox = New message_box_frm(messageTxt2, translations.getText("exclamantion"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation, Me.Location.X + Me.Width / 2, Me.Location.Y + Me.Height / 2)
            msgbox.ShowDialog()
            Exit Sub
        End If
        ...
        msgbox = New message_box_frm(messageTxt, translations.getText("question"), MessageBoxButtons.OKCancel, MessageBoxIcon.Question, Me.Location.X + Me.Width / 2, Me.Location.Y + Me.Height / 2)
        If (msgbox.ShowDialog = DialogResult.OK) Then
            enableButtonsAndLinks(Me, False)
    ...
    Private Sub download_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles download.LinkClicked
        translations.load("commonForm")
        ...
        If (saveFileDialog1.ShowDialog() = DialogResult.OK) Then
            ...
            If cfgFile.Exists Then
                ...
                Catch
                    ...
                    translations.load("messagebox")
                    msgbox = New message_box_frm(msgTxt, translations.getText("exclamantion"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation, Me.Location.X + Me.Width / 2, Me.Location.Y + Me.Height / 2)
                    msgbox.ShowDialog()
                    Exit Sub
                End Try
    ...
    Private Sub qrcode_img_Click(sender As Object, e As EventArgs) Handles qrcode_img.Click
        If (listview_works.SelectedItems.Count > 0) Then
            ...
            Else
                ...
                translations.load("messagebox")
                msgbox = New message_box_frm(message3 & ". ", translations.getText("exclamantion"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation, Me.Location.X + Me.Width / 2, Me.Location.Y + Me.Height / 2)
                msgbox.ShowDialog()
            End If
        End If
    End Sub
    ...
    Private Sub saveNewCard_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles saveNewCard.LinkClicked
        initializeSmartCard.siteForm = Nothing
        initializeSmartCard.workersForm = New workers_frm
        ...
        If (limosaList.SelectedItems.Count > 0) Then
            ...
            Else ' edit limosa
                ...
                If (limosasDB.Item("file")(limosaList.SelectedIndices(0) - 1).Equals("")) Then
                    limosasImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileMissingSmall.fw.png"))
                Else
                    limosasImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileokSmall.fw.png"))
                End If
                ...
                If (limosasDB.Item("qrcode")(limosaList.SelectedIndices(0) - 1).Equals("")) Then
                    qrcode_img.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("searchqrcode.png"))
                Else
                    qrcode_img.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("loading.png"))
                    qrcode_img.SizeMode = PictureBoxSizeMode.StretchImage
                    Dim tClient As WebClient = New WebClient
                    Try
    ...
    Private Sub listview_works_SelectedIndexChanged_1(sender As Object, e As EventArgs) Handles listview_works.SelectedIndexChanged
        ...
        If (listview_works.SelectedItems.Count > 0) Then
            ...
            Else 'edit worker
                ...
                Else
                    enableDel = True
                    contratoImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileokSmall.fw.png"))
                ...
                Else
                    ...
                    destacamentoImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileokSmall.fw.png"))
                End If
                ...
                Else
                    enableDel = True
                    actdel.Enabled = True
                    actImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileokSmall.fw.png"))
                End If
                ...
                Else
                    enableDel = True
                    a1del.Enabled = True
                    a1Img.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileokSmall.fw.png"))
                End If
                ...
                Else
                    ...
                    mutuelleImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileokSmall.fw.png"))
                End If
                ...
                Else
                    ...
                    medicoImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileokSmall.fw.png"))
                End If
                ...
                Else
                    ...
                    gruistaImg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileokSmall.fw.png"))
                End If
                ...
                Else
                    enableDel = True
                    ccdel.Enabled = True
                    ccimg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileokSmall.fw.png"))
                End If
                ...
                Else
                    ...
                    csaudeimg.Image = Image.FromFile(MainMdiForm.state.imagesPath & Convert.ToString("fileokSmall.fw.png"))
                End If
