...
Public Class NFCard
    ...
    Public RdrState As NFCard.SCARD_READERSTATE
    Public pioSendRequest As NFCard.SCARD_IO_REQUEST
    ...
    End Function
    <DllImport("winscard.dll")>
    Public Shared Function SCardStatus(ByVal hCard As Integer, ByVal szReaderName As String, ByRef pcchReaderLen As Integer, ByRef State As Integer, ByRef Protocol As Integer, ByRef ATR As Byte, ByRef ATRLen As Integer) As Integer
    ...
    End Function
    <DllImport("winscard.dll")>
    Public Shared Function SCardState(ByVal hCard As Integer, ByRef State As UInteger, ByRef Protocol As UInteger, ByRef ATR As Byte, ByRef ATRLen As UInteger) As Integer
    ...
    Public Function getStatus()
        ...
        retCode = SCardStatus(hCard, szReaderList, 255, State, Protocol, ATR, ATRLen)
        Select Case State
            Case SCARD_ABSENT
    ...
    Public Function SelectDevice() As Boolean
        ...
        If Not IsNothing(availableReaders) AndAlso availableReaders.Count > 0 Then
            RdrState = New NFCard.SCARD_READERSTATE()
    ...
    Public Function ListReaders() As List(Of String)
        ...
        If retCode <> NFCard.SCARD_S_SUCCESS Then
            errorMessage = GetScardErrMsg(retCode)
            Return AvailableReaderList
        End If

        Dim ReadersList As Byte() = New Byte(ReaderCount - 1) {}
        retCode = NFCard.SCardListReaders(hContext, Nothing, ReadersList, ReaderCount)
    ...
    Public Function connectCard() As Boolean
        connActive = True
        retCode = NFCard.SCardConnect(hContext, readername, NFCard.SCARD_SHARE_SHARED, NFCard.SCARD_PROTOCOL_T0 Or NFCard.SCARD_PROTOCOL_T1, hCard, Protocol)
        If retCode <> NFCard.SCARD_S_SUCCESS Then
            errorMessage = NFCard.GetScardErrMsg(retCode)
            connActive = False
            Return False
        End If
    ...
    Public Function readCardUID() As Boolean
        ...
        Dim request As NFCard.SCARD_IO_REQUEST = New NFCard.SCARD_IO_REQUEST()
        request.dwProtocol = NFCard.SCARD_PROTOCOL_T1
        request.cbPciLength = System.Runtime.InteropServices.Marshal.SizeOf(GetType(NFCard.SCARD_IO_REQUEST))
        Dim sendBytes As Byte() = New Byte() {&HFF, &HCA, &H0, &H0, &H0}
        Dim outBytes As Integer = receivedUID.Length
        Dim status As Integer = NFCard.SCardTransmit(hCard, request, sendBytes(0), sendBytes.Length, request, receivedUID(0), outBytes)

        If status <> NFCard.SCARD_S_SUCCESS Then
            cardUID = "Error"
            Return False
        Else
    ...
    Public Function RemoveEmptyBytes16DigitsDecimal(ByVal packet As Byte()) As Byte()
