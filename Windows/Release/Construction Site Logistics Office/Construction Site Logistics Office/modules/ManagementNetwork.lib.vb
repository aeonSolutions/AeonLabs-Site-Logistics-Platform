...
Public Module ManagementNetwork
    ...
    Public Function IsResponseOk(response As String) As Boolean
        If response.Equals("") Then
            MainMdiForm.statusMessage = "Empty response"
            Return False
        End If
        Try
            Dim jsonResult = JsonConvert.DeserializeObject(Of Dictionary(Of String, Object))(response)
            Return Not jsonResult.Item("error")
        Catch ex As Exception
            saveCrash(ex)
            MainMdiForm.statusMessage = "Error getting server error code: [" & response & "]"
            Return False
        End Try
    ...
    'DISCONTINUATED FUNCTIONS
    Public Function ConvertDataToArray(response As String) As String(,)
        Dim d As String(,)
        ReDim d(1, 1)
        d(0, 0) = ""
        d(0, 1) = ""
        ...
        If (IsResponseOk(response)) Then
            Dim jsonResult = JsonConvert.DeserializeObject(Of Dictionary(Of String, Object))(response)
            ReDim d(jsonResult.Item("data").Count, jsonResult.Item("data").item(0).Count)
            d(0, 0) = ""
            For i = 1 To jsonResult.Item("data").Count
                For j = 1 To jsonResult.Item("data").item(i - 1).Count
                    d(0, j) = ""
                    d(1, 0) = ""
                    d(i, j) = jsonResult.Item("data").item(i - 1).item(j - 1).ToString
    ...
    Public Function GetHttpFile(url As String) As String
        MainMdiForm.statusMessage = "Function descontinuated. UPDATE CODE"
        Return ""
    End Function

    Public Function SendHttpFile(url As String, file As String) As String
        MainMdiForm.statusMessage = "Function descontinuated. UPDATE CODE"
        Return ""
    End Function


    Public Function NetworkRequest(outside As Boolean, type As String, url As String) As String
        MainMdiForm.statusMessage = "Function descontinuated. UPDATE CODE"
        Return ""
    End Function
End Module
