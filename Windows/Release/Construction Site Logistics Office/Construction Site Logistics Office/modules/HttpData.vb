...
Public Class HttpData
    ...
    Public Function RequestExternalData(url As String) As String
        ...
        If Application.OpenForms().OfType(Of MainMdiForm).Any Then
        ...
        Catch ex As Exception
            translations.load("errorMessages")
            If Application.OpenForms().OfType(Of MainMdiForm).Any Then
                MainMdiForm.statusMessage = translations.getText("contactingCommServer") & ". MSG:" & ex.Message.ToString '& " URL: " & url
            End If
        ...
        If Application.OpenForms().OfType(Of MainMdiForm).Any Then
            MainMdiForm.statusMessage = url.Replace(Environment.NewLine, " ")
        End If
        Return utf8Encoding.GetString(result)

    End Function

    Public Function SendHttpFile(vars As Dictionary(Of String, String), file As String, ByVal Optional origin As String = "office") As String
        If Not System.Net.NetworkInformation.NetworkInterface.GetIsNetworkAvailable() Then
            Return "{'error':true,'message':'" & translations.getText("errorNoNetwork") & "'}"
        End If
        ...
        PostData.Add("origin", origin)
        PostData.Add("data", encrypted)

        translations.load("commonForm")
        MainMdiForm.statusMessage = translations.getText("sendingFile")
        upload_frm.postData = PostData
        upload_frm.address = url
        upload_frm.fileName = file
        upload_frm.ShowDialog()
        Dim responseFromServer As String = System.Text.Encoding.UTF8.GetString(upload_frm.responseBytes)
        Dim decrypted As String = ""
        Try
            If IsBase64String(responseFromServer) And Not responseFromServer.Equals("") Then
        ...
        Catch ex As Exception
            translations.load("errorMessages")
            decrypted = "{'error':true,'message':'" & translations.getText("errorDecryptingData") & " (" & ex.Message.ToString.Replace("'", "\'") & ")'}"
        End Try
        Return decrypted
    End Function

    Public Function GetHttpFile(vars As Dictionary(Of String, String), ByVal Optional origin As String = "office") As String
        If Not System.Net.NetworkInformation.NetworkInterface.GetIsNetworkAvailable() Then
            Return "{'error':true,'message':'" & translations.getText("errorNoNetwork") & "'}"
        End If
        ...
        translations.load("commonForm")
        MainMdiForm.statusMessage = translations.getText("receivingFile")

        DownloadForm.address = MainMdiForm.state.ServerBaseAddr & MainMdiForm.state.ApiServerAddrPath & "?" & url
        DownloadForm.postData = PostData
        DownloadForm.ShowDialog()

        translations.load("readyMessages")
        MainMdiForm.statusMessage = translations.getText("downloadCompleted")
        Dim result = System.Text.Encoding.UTF8.GetString(DownloadForm.responseBytes)
        Return result
    End Function
